...
- 支持交互式选择供应商（上下键切换、回车确认），同名供应商自动提示选择
...
└── create-shortcut.ps1    # PowerShell 脚本，为指定供应商创建桌面快捷方式
```

## 前置要求

- Node.js 22.13 或更高版本（使用 `node:sqlite` 内置模块，实验性警告已自动屏蔽）
- 已安装 CC-Switch 桌面版并配置过至少一个 `claude` 类型供应商
...
运行脚本，会在项目目录下生成 `<ProviderName>.lnk`：

```powershell
powershell -ExecutionPolicy Bypass -File .\create-shortcut.ps1
```

双击快捷方式即可用指定供应商启动 Claude Code。
...
2. **解析供应商**：
    - 未传名称 -> 列出所有 `claude` 类型供应商交互选择
    - 找到唯一匹配 -> 直接使用
    - 找到多个同名 -> 交互选择具体项
    - 未找到 -> 回退到交互选择（最多重试 3 次）
3. **提取环境变量**：解析供应商的 `settings_config`，提取 `env` 字段，过滤掉系统关键变量（`PATH`、`HOME`、`USERPROFILE` 等）和非字符串值
4. **写 settings 文件**：将 env 写入 `settings/settings_<id>.json`
5. **启动 Claude**：执行 `claude --settings <file>`，额外参数透传
